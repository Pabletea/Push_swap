...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/04 13:57:53 by pabalons          #+#    #+#             */
/*   Updated: 2025/02/04 13:58:19 by pabalons         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (b)
	{
		...
		while (current_a) //While the pointer is not set to NULL
		{
			if (current_a->data > b->data 
				&& current_a->data < best_match_index) //Check if the `a` node's value is bigger than `b` node, && smaller than the "closest bigger" so far
			{
				best_match_index = current_a->data; //Set the best match as the value in the current `a` node
