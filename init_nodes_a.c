...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/04 13:53:22 by pabalons          #+#    #+#             */
/*   Updated: 2025/02/04 13:55:21 by pabalons         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_target_a(t_stack *a, t_stack *b)//Find `a` node's target in stack `b`
{
	...
	while (a) //As long as we have nodes in stack `a`
	{
		...
		if (best_match_index == LONG_MIN) //Check if the LONG_MIN hasn't changed, it means we haven't found anything smaller
			a->target_node = find_max(b); //If so, find the biggest `data` and set this as the target node
		else
...
void	set_cheapest(t_stack *stack) //Define a function that sets a node's `cheapest` attribute as `true` or `false`
{
	...
	cheapest_node->is_cheapiest = 1; //After iterating through the stack, if no cheaper node is found than the current, then set the cheapest node's `cheapest` attribut to `true` in the data structure
}
